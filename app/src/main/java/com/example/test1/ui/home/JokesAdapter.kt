...
class JokesAdapter(
    private val shareClickListener: (Joke) -> Unit,
    private val likeClickListener: (Joke) -> Unit
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return ViewHolder(parent, shareClickListener, likeClickListener)
    }
...
class ViewHolder(
    ...
    private val shareClickListener: (Joke) -> Unit,
    private val likeClickListener: (Joke) -> Unit
) : RecyclerView.ViewHolder(parent inflate R.layout.item_jokes) {

    private val tvMessage = itemView.findViewById<TextView>(R.id.tv_message)
    private val btnShare = itemView.findViewById<Button>(R.id.btn_share)
    private val btnLike = itemView.findViewById<Button>(R.id.btn_like)

    fun bind(item: Joke) {
        tvMessage.text = item.joke
        btnLike.setOnClickListener {
            likeClickListener.invoke(item)
        }
        btnShare.setOnClickListener {
            shareClickListener.invoke(item)
        }
    }
}
